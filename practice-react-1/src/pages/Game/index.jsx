...
export function Game({ data = {}, setData = () => {} }) {
  ...
  function updateGameScore(isTeamOne) {
    ...
    function wonGame() {
      if (gameMode == REGULAR_MODE) return updatedGameScore.teamOne == 4 || updatedGameScore.teamTwo == 4
      const { teamOne, teamTwo } = updatedGameScore

      if (gameMode == TIEBRAKE_MODE) return Math.abs(teamOne - teamTwo) >= 2 && (updatedGameScore.teamOne == 7 || updatedGameScore.teamTwo == 7)
      if (gameMode == SUPERTIEBRAKE_MODE) return Math.abs(teamOne - teamTwo) >= 2 && (updatedGameScore.teamOne == 10 || updatedGameScore.teamTwo == 10)
    }
    ...
    if (wonGame()) {
      ...
      const nextGameIsTieBrake = gameMode == REGULAR_MODE && updatedSetScore.teamOne == 6 && updatedSetScore.teamTwo == 6
      ...
      function setWon() {
        if (gameMode == REGULAR_MODE) return Math.abs(updatedSetScore.teamOne - updatedSetScore.teamTwo) >= 2 && (updatedSetScore.teamOne == 6 || updatedSetScore.teamTwo == 6)
      ...
      if (setWon()) {
        ...
        const nextIsSuperTieBrake = data.currentSet == 2 && bestOfThreeSets && lastSetIsSupertiebrake
  ...
  return (
    <Page className={styles.game}>
      ...
      <Col className="mt-4 d-flex align-items-center justify-content-between">
        <button onClick={() => updateGameScore(true)} className="btn btn-primary">Ponto Time 1</button>

        <button onClick={() => updateGameScore(false)} className="btn btn-primary">Ponto Time 2</button>
